...
Angular and the legacy frontend coexist through explicit per-surface ownership. During Phase 3 all product routes are registered in Angular for discoverability, but their owner is `legacy`; Angular renders an ownership boundary, not a substitute feature.
