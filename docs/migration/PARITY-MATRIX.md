...
| Damage & Healing | Golden bundle and report telemetry | report/telemetry graphs | **Fixture leak** when report telemetry is unavailable | empty, stored, connected, partial graph, missing phase, error | Values and denominators trace to WCL payload; visual graph parity | Medium |
...
| Composition | Golden plus roster intelligence | report CombatantInfo/roster | Partial and visually owned by Golden when offline | empty, partial roster, role groups, missing gear/talents, Wowhead unavailable | Real WCL actors only; safe unknowns; no fictional role balance | Medium |
